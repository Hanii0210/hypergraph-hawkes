"""
Experiment 18: RET-1 window-stability analysis under the same real-data protocol.

This is an additive experiment script. It does not modify exp10 or any existing
experiment. It mirrors exp16/exp17 but loads CRCNS ret-1 MATLAB files.

Default target:
    20080516_R1.mat, record 0, because this is the small N=7 ret-1 recording
    close to the original real-data example.

Protocol:
    1. Load one ret-1 .mat file and one recording column from spikes.
...
Quick run:
    python experiments/exp18_ret1_window_stability.py --raw-root data/raw --top-m-list 1 --n-iter 20

Fuller run:
    python experiments/exp18_ret1_window_stability.py --raw-root data/raw --top-m-list 1,2,3 --n-iter 20
"""
...
def find_ret1_mat(raw_root: Path, mat_substring: str) -> Path:
    matches = sorted(raw_root.glob(f"**/*{mat_substring}*.mat"))
    matches = [m for m in matches if "__MACOSX" not in m.parts and not m.name.startswith("._")]
    if not matches:
        raise FileNotFoundError(
            f"Could not find ret-1 .mat containing '{mat_substring}' under {raw_root}. "
            "Example expected path: data/raw/crcns_ret-1/Data/20080516_R1.mat"
        )
    return matches[0]


def load_ret1_record(raw_root: Path, mat_substring: str, record_index: int):
    mat_path = find_ret1_mat(raw_root, mat_substring)
    mat = sio.loadmat(mat_path, simplify_cells=True)

    if "spikes" not in mat:
        raise KeyError(f"No 'spikes' variable found in {mat_path}. Keys: {list(mat.keys())}")

    spikes = mat["spikes"]

    # ret-1 usually stores spikes as object array with shape (n_cells, n_records).
    if isinstance(spikes, np.ndarray) and spikes.dtype == object:
        if spikes.ndim == 2:
            n_cells, n_records = spikes.shape
            if record_index < 0 or record_index >= n_records:
                raise IndexError(
                    f"record_index={record_index} out of range for spikes shape {spikes.shape}"
                )
            trains = [as_1d_float(spikes[i, record_index]) for i in range(n_cells)]
        elif spikes.ndim == 1:
            # Either one record with cells, or a list of records. Try to treat as cells.
            trains = [as_1d_float(spikes[i]) for i in range(spikes.shape[0])]
            n_records = 1
        else:
            raise ValueError(f"Unsupported spikes ndim={spikes.ndim}, shape={spikes.shape}")
    elif isinstance(spikes, list):
        # Rare simplified structure.
        if len(spikes) > 0 and isinstance(spikes[0], (list, tuple, np.ndarray)):
            trains = [as_1d_float(x) for x in spikes]
            n_records = 1
        else:
            raise ValueError("Unsupported list structure for spikes")
    else:
        raise ValueError(f"Unsupported type for spikes: {type(spikes)}")

    trains = [t for t in trains if len(t) > 0]
    if not trains:
        raise ValueError(f"No nonempty spike trains found in {mat_path}, record {record_index}")

    datainfo = mat.get("datainfo", {})
    info = {}
    if isinstance(datainfo, dict):
        for key in ["date", "Ncell", "RecNo", "SmplNo", "description", "animal"]:
            if key in datainfo:
                try:
                    info[key] = datainfo[key].item() if hasattr(datainfo[key], "item") else datainfo[key]
                except Exception:
                    info[key] = str(datainfo[key])

    return mat_path, trains, info


def select_fixed_subset(trains, top_n, span_start, span_end):
    counts = []
    ...
    for new_node, (old_idx, count) in enumerate(selected):
        mapping.append({
            ...
            "original_cell_index": int(old_idx),
...
def edge_to_original(edge, mapping):
    return tuple(mapping[i]["original_cell_index"] for i in edge)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--raw-root", default="data/raw")
    parser.add_argument("--mat-substring", default="20080516_R1")
    ...
    mat_path, trains, info = load_ret1_record(raw_root, args.mat_substring, args.record_index)

    top_n = min(args.top_n, len(trains))
    selected_indices, mapping = select_fixed_subset(
        trains, top_n, span_start, span_end
    )
    ...
    print("=" * 114)
    print("Experiment 18: RET-1 window-stability analysis")
    print("=" * 114)
    print(f"source_file     : {mat_path}")
    print(f"mat_substring   : {args.mat_substring}")
    print(f"record_index    : {args.record_index}")
    print(f"datainfo        : {info}")
    print(f"analysis span   : [{span_start}, {span_end}) sec")
    print(f"starts          : {starts}")
    print(f"duration        : {args.duration}")
    print(f"top_n fixed     : {top_n}")
    ...
    for m in mapping:
        print(f"  node {m['new_node']:2d} <- original cell {m['original_cell_index']:2d}, "
    ...
    for w_idx, start in enumerate(starts):
        ...
        for top_m in top_m_list:
            ...
            mu_sel, ap_sel, ll_sel = fit_pairwise(
                sel, T_sel, top_n, kernel, anchor_calc,
            ...
            mu_pw_i, ap_pw_i, ll_pw_i = fit_pairwise(
                inf, T_inf, top_n, kernel, anchor_calc,
                args.n_iter, args.lambda_l1, seed + 1
            )

            mu_hth_i, ap_hth_i, ah_i, hist_i = fit_hth(
                inf, T_inf, top_n, candidates, kernel, anchor_calc,
                mu_pw_i, ap_pw_i,
                n_iter=args.n_iter,
            ...
            row = {
                "dataset": "ret-1",
                "mat_substring": args.mat_substring,
                ...
                "candidate_edges_original_cells": ";".join(str(edge_to_original(e, mapping)) for e in candidates),
                "logL_pairwise_heldout": float(ll_pw_i),
                "logL_HTH_heldout": float(ll_hth_i),
                "delta_L_heldout": dL,
                "bicdiff_candidate_count": bic_candidate,
                "main_edge": main_edge,
                "main_edge_original_cells": str(edge_to_original(main["edge"], mapping)),
    ...
    safe_name = args.mat_substring.replace("/", "_").replace("\\", "_")
    out_prefix = Path(args.out_prefix) if args.out_prefix else Path(
        f"experiments/exp18_ret1_{safe_name}_rec{args.record_index}_window_stability"
    )
